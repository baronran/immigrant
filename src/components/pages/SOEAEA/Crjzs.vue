<template :key="keys">
  <cal title="出入境证件" type="1">
    ...
    <template v-slot:right>
      <div class="right-nav cursor-event" :class="{'on':typeStatus==idex}"  @click="typeChange(idex)" v-for="(item,idex) in ['因私护照','港澳通行证','台湾通行证']">{{item}}</div>
...
import apiJson from "../../../../public/apiJson/Crjzs"
...
const renderChart = () => {
  ...
  let carNum = []; //中国公民检查集合
  let yearOnYear = []; //外国人集合
  let time = []; //时间集合
apiJson.data.map((index)=>{
   carNum.push(index.carNum)
   yearOnYear.push(index.yearOnYear)
   time.push(index.city)

})

  let chartObj = {
    series: [
      {name: '证件数', data: carNum},
      {name: '同比', data:yearOnYear},
    ],
    chartList: time,
  };
