...
let route = useRoute()
let isSSO =  route.name == 'BureauSSO'
...
let getUrl = function(index){
  return new URL('../../../assets/img/page/SSO/rt'+ (isSSO?'':'1')+index +'.png', import.meta.url).href
}
let pageData = ref([])

pageData.value = isSSO?[
...
let title = ref(isSSO?'出入境证件签发':'查疑查假查伪')
